...
class Trainer:
    def __init__(self, configs):
        ...
        torch.manual_seed(5)
        self.network = TAU(configs).to(configs.device)
    ...
    def train_once(self, input_sst, sst_true):
        sst_pred = self.network(input_sst.float())
        self.opt.zero_grad()
        ...
        loss = self.loss(sst_pred, sst_true.float().to(self.device)) + self.configs.alpha * self.diff_div_reg(sst_pred, sst_true.float().to(self.device))
        loss.backward()
    ...
    def test(self, dataloader_test):
        sst_pred = []
        with torch.no_grad():
            for input_sst, _ in dataloader_test:
                sst = self.network(input_sst.float())
                sst_pred.append(sst)
...
if __name__ == '__main__':
    ...
    # model = TAU(configs).to(configs.device)
    # net = torch.load('checkpoint_51.44337749481201.chk')
    ...
    # np.savez('result51.44337749481201.npz', pred=pred.cpu(), label=label.cpu())
    ...
    device = configs.device
    model = TAU(configs).to(configs.device)
    tensor = (torch.rand(1, 24, 4, 40, 40).to(device))
