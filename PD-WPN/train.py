...
class Trainer:
    ...
    def train_once(self, input_sst, sst_true, ssr_ratio):
        ...
        if self.epoch<self.reg_epoch:
            mom_loss = moment_regularizer(self.network, K2M_class=K2M, q=2, lamb=1e-3, target_scale=1.0) + gradient_first_order_regularizer(model, K2M_class=K2M, lamb=1e-3, target_scale=1.0, suppress_second=self.configs.suppress_second)
            loss = loss + mom_loss
        loss.backward()
