...
class SwinLSTMNET(nn.Module):
    def __init__(self, configs):
        super(SwinLSTMNET, self).__init__()

        self.configs = configs

        new_size = self.next_power_of_2(configs.img_size[0])
        if self.configs.img_size[0] != new_size:
            print('Please set img_size in the config file to （' + str(new_size) + ',' + str(new_size) + '） and run again!')
            sys.exit()

        self.net = SwinLSTM(configs.img_size, configs.patch_size, configs.in_chans, configs.embed_dim, configs.depths_downsample, configs.depths_upsample, configs.num_heads, configs.window_size)
    ...
    def pad_to_next_power_of_2_tensor(self,input_tensor):
        # 获取输入张量的形状
        B, T, C, H, W = input_tensor.shape

        # 假设 H == W
        new_size = self.next_power_of_2(H)


        # 如果 H 和 W 已经是 2 的幂且不需要 padding，直接返回
        if new_size == H:
            return input_tensor
        ...
        # PyTorch 的 pad 按顺序是 (left, right, top, bottom)，因此需要调整顺序
        # 我们只需要在最后两个维度（H, W）上进行 padding
        padded_tensor = torch.nn.functional.pad(input_tensor, (0, pad_size, 0, pad_size), mode='constant', value=0)

        return padded_tensor

    def remove_padding_tensor(self,padded_tensor, original_shape):
        # 获取原始尺寸
        _, _, _, original_H, original_W = original_shape
    ...
    def forward(self,inputs,train=True):
        ...
        inputs = inputs.to(self.configs.device)

        original_shape = inputs.shape

        inputs = self.pad_to_next_power_of_2_tensor(inputs)

        states_down = [None] * len(self.configs.num_layers)
        states_up = [None] * len(self.configs.num_layers)
        ...
        for i in range(inputs_len - 1):
            output, states_down, states_up = self.net(inputs[:, i], states_down, states_up)
            outputs.append(output)

        for i in range(self.configs.output_length):
            output, states_down, states_up = self.net(last_input, states_down, states_up)
            outputs.append(output)
            last_input = output

        next_frames = torch.stack(outputs, dim=1).contiguous()

        if train:
            next_frames = next_frames
        else:
            next_frames = next_frames[:,-self.configs.output_length:]

        next_frames = self.remove_padding_tensor(next_frames, original_shape)

        return next_frames

if __name__ == '__main__':
    x = torch.randn((2, 24, 4, 40, 40)).to(configs.device)
    y = torch.randn((2, 24, 4, 40, 40)).to(configs.device)
    model1 = SwinLSTMNET(configs).to(configs.device)
    output = model1(x,train=False)
