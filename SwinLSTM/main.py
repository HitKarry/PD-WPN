...
from SwinLSTM_D import SwinLSTMNET
...
class Trainer:
    def __init__(self, configs):
        ...
        torch.manual_seed(5)
        self.network = SwinLSTMNET(configs).to(configs.device)
    ...
    def train_once(self, input_sst, sst_true):
        sst_pred = self.network(input_sst.float(), train=True)
        self.opt.zero_grad()
    ...
    def test(self, dataloader_test):
        ...
        with torch.no_grad():
            for input_sst, target_sst in dataloader_test:
                sst = self.network(input_sst.float(), train=False)
                sst_pred.append(sst)
...
if __name__ == '__main__':
    ...
    model = SwinLSTMNET(configs).to(configs.device)
    net = torch.load('checkpoint_89.01648330688477.chk')
    ...
    np.savez('result89.01648330688477.npz', pred=pred.cpu(), label=label.cpu())