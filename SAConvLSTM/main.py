...
class Trainer:
    ...
    def train_once(self, input_sst, sst_true, ssr_ratio):
        sst_pred = self.network(input_sst.float(), teacher_forcing=True,
    ...
    def train(self, dataset_train, dataset_eval, chk_path):
        ...
        for i in range(self.configs.num_epochs):
            ...
            for j, (input_sst, sst_true) in enumerate(dataloader_train):
                ...
                loss_z500, loss_t850, loss_t2m, loss_new, loss  = self.train_once(input_sst, sst_true, ssr_ratio)  # y_pred for one batch
...
if __name__ == '__main__':
    ...
    # net = torch.load('checkpoint_49.90481472015381.chk')
    ...
    # np.savez('result49.90481472015381.npz', pred=pred.cpu(), label=label.cpu())
