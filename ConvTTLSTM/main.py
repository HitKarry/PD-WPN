...
class Trainer:
    ...
    def train_once(self, input_sst, sst_true, ssr_ratio):
        sst_pred = self.network(input_sst.float().to(self.device), teacher_forcing=True,
    ...
    def test(self, dataloader_test):
        ...
        with torch.no_grad():
            for input_sst, _ in dataloader_test:
                sst = self.network(input_sst.float().to(self.device), train=False)
                sst_pred.append(sst)
    ...
    def train(self, dataset_train, dataset_eval, chk_path):
        ...
        for i in range(self.configs.num_epochs):
            ...
            for j, (input_sst, sst_true) in enumerate(dataloader_train):
                ...
                loss_z500, loss_t850, loss_t2m, loss_new, loss  = self.train_once(input_sst.to(), sst_true, ssr_ratio)  # y_pred for one batch

                if (j+1) % self.configs.display_interval == 0:
                    print('batch training loss: {:.2f}, {:.2f}, {:.2f}, {:.2f}, {:.2f}, ssr: {:.5f}'.format(loss_z500, loss_t850, loss_t2m, loss_new, loss, ssr_ratio))
...
if __name__ == '__main__':
    ...
    #
    # train_x = np.concatenate([data_x[:8406],data_y[:8406]],1)
    # train_y = data_y[:8406]
    ...
    #                                  configs.bias, configs.hidden_channels, configs.layers_per_block,
    #                                  configs.skip_stride, configs.input_dim, configs.output_dim).to(configs.device)
    # net = torch.load('checkpoint_51.87372016906738.chk')
    ...
    # np.savez('result51.87372016906738.npz', pred=pred.cpu(), label=label.cpu())
    ...
    model = ConvTTLSTMNet(configs.order, configs.steps, configs.ranks, configs.kernel_size,
                                     configs.bias, configs.hidden_channels, configs.layers_per_block,
                                     configs.skip_stride, configs.input_dim, configs.output_dim).to(configs.device)
    tensor = (torch.rand(1, 48, 4, 40, 40).to(device))
