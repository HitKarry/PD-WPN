...
class Trainer:
    ...
    def loss_sst(self, y_pred, y_true):
        ...
        rmse = torch.mean((y_pred[:, -26:] - y_true[:, -26:])**2, dim=[2, 3]).to('cuda:0')
        rmse_sequence_length = rmse.shape[-1]
        alpha = [np.log1p(i) for i in range(1, rmse_sequence_length + 1)]
        alpha = torch.reshape(torch.tensor(alpha), (1, rmse_sequence_length)).to('cuda:0')
        rmse = torch.sqrt(torch.mean(rmse * alpha, dim=0)).to('cuda:0')
        return rmse.sum()
    ...
    def train(self, dataset_train, dataset_eval, chk_path):
        ...
        for i in range(self.configs.num_epochs):
            ...
            print('epoch train loss: {:.5f}\nepoch eval loss: sst: {:.2f}, nino: {:.2f}, sc: {:.4f}'.format(losses / s,loss_sst_eval, loss_nino_eval, sc_eval))
            ...
            if sc_eval <= best:
                ...
                self.save_model(chk_path+'_'+str(i)+'_'+str(j)+'_'+str(sc_eval))
                best = sc_eval
...
if __name__ == '__main__':
    ...
    print('\nreading data')
    sst_train, nino_train, sst_eval, nino_eval, sst_test, nino_test = prepare_data('C:/PythonProject/enso/enso_round1_train_20210201')
    ...
    trainer.save_configs('config_train')
    trainer.train(dataset_train, dataset_eval, 'checkpoint')
    ...
    #                       configs.bias, configs.hidden_channels, configs.layers_per_block,
    #                       configs.skip_stride, configs.input_dim, configs.output_dim).to(configs.device)
    # net = torch.load('checkpoint_7_1040_27.18083832678647')
    # model.load_state_dict(net['net'])
    # model.eval()
    # dataloader_eval = DataLoader(dataset_eval, batch_size=100, shuffle=False)
    # with torch.no_grad():
    #     for j, (input_sst, nino_true) in enumerate(dataloader_eval):
    #         _, nino_pred = model(input_sst.float().to(device), train=False)
    #         if j == 0:
    #             pred = nino_pred
    #             label = nino_true
    #         else:
