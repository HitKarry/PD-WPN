...
class ConvTTLSTMCell(nn.Module):
    def __init__(self, input_channels, hidden_channels, order, steps, ranks,
        ...
        for l in range(order):
            self.layers.append(Conv2d(
                ...
                out_channels=ranks if l < order - 1 else 4 * hidden_channels).to(configs.device))

            self.layers_.append(Conv3d(in_channels=hidden_channels, out_channels=ranks))

        self.layers = self.layers.to(configs.device)
    ...
    def forward(self, inputs, first_step=False):
        ...
        for l in range(self.order):
            ...
            input_states = self.layers_[l](input_states.to(configs.device))
            input_states = torch.squeeze(input_states, dim=-1)

            if l == 0:
                temp_states = input_states
            else:
                temp_states = input_states.to(configs.device) + self.layers[l-1](temp_states.to(configs.device))

        concat_conv = self.layers[-1](torch.cat([inputs.to(configs.device), temp_states.to(configs.device)], dim=1).to(configs.device))
        ...
        self.cell_states = f.to(configs.device) * self.cell_states.to(configs.device) + i.to(configs.device) * g.to(configs.device)
...
class ConvTTLSTMNet(nn.Module):
    def __init__(self, order, steps, ranks, kernel_size, bias,
        ...
        self.layers = nn.ModuleDict().to(configs.device)
        ...
        self.layers["output"] = nn.Conv2d(channels, 4, kernel_size=1)

    def forward(self, input_x, device=torch.device('cuda:0'), input_frames=24, future_frames=24, output_frames=47,
        ...
        # input_x = input_x[:, :, None] # (N, T, C, H, W)
        if train:
        ...
        total_steps = input_frames + future_frames - 1 #37
        ...
        for t in range(total_steps):
            if t < input_frames:
                ...
                input_ = input_x[:, t].to(device) * mask + outputs[t-1].to(device) * (1 - mask)
            first_step = (t == 0)
            input_ = input_.float()
        ...
        outputs = torch.stack(outputs, dim=1)[:, -future_frames:]
