...
class Trainer:
    def __init__(self, configs):
        ...
        torch.manual_seed(5)
        self.network = MIM_Model(configs).to(configs.device)
...
if __name__ == '__main__':
    ...
    model = MIM_Model(configs).to(configs.device)
    net = torch.load('checkpoint_49.20514965057373.chk')
    ...
    np.savez('result49.20514965057373.npz', pred=pred.cpu(), label=label.cpu())